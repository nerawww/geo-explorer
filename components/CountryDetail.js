...
export function displayCountryName(country) {
  document.getElementById(
    "countryName"
  ).textContent = `Région : ${country.name.common}`;
}
...
export function displayFlag(country) {
  document.getElementById(
    "flagContainer"
  ).innerHTML = `<img src="${country.flags.svg}" alt="Flag of ${country.name.common}" width="100">`;
}
...
export function displayCapital(country) {
  document.getElementById(
    "capitalContainer"
  ).textContent = `Capitale : ${country.capital[0]}`;
}
...
export function displayRegion(country) {
  document.getElementById(
    "regionContainer"
  ).textContent = `Région : ${country.region}`;
}
...
export function displayLanguages(country) {
  document.getElementById(
    "languageContainer"
  ).textContent = `Langues : ${Object.values(country.languages).join(", ")}`;
}
...
export function displayCurrency(country) {
  ...
  document.getElementById(
    "monneyContainer"
  ).textContent = `Monnaie : ${currencies}`;
}
...
export function displayArea(country) {
  document.getElementById(
    "areaContainer"
  ).textContent = `Superficie : ${country.area.toLocaleString()} km²`;
}
...
export function displayDensity(country) {
  ...
  document.getElementById(
    "densityContainer"
  ).textContent = `Densité : ${density} hab/km²`;
}
...
export function displayMap(country) {
  ...
  document.getElementById(
    "infoContentRight"
  ).innerHTML = `<iframe width="500" height="500" src="https://www.openstreetmap.org/export/embed.html?bbox=${
    lng - bboxLngDelta
  }%2C${lat - bboxLatDelta}%2C${lng + bboxLngDelta}%2C${
    lat + bboxLatDelta
